...
- One authored moment: the carriage sweep that draws the traces in 2400ms and stamps each mark as the pen reaches it, with a replay control, a reduced-motion settle and a print settle.
...
Responsive behavior steps at 1349 (with 1101, where the chapter rail narrows to 200px), 1280 (with its 1279 twin, where the margin index takes its column and, below, stacks above the prose — and where the strip's sheet stops stacking), 1100 (the docs collapse), 900 (where the sheet's two ranges meet) and 620. At ≤1279px the margin index leaves the margin and becomes a compact bordered list above the prose (`repeat(auto-fit, minmax(220px, 1fr))`). At ≤1100px the docs collapse to a single column and the rail becomes static, full width above the article, with a 2px bottom rule replacing the margin rule. At ≤900px the instrument stacks the legend above the strip (the 2px divider becomes a bottom rule), band padding drops to 72px, and plate rows stack into term-over-definition. At ≤620px body text drops to 1rem, mono drops again (0.6875rem inside runlogs and windows, 0.75rem in command lines), the legend reverts from graphite to the surface it stands on (its dark mass would swallow a phone screen), the rule list drops to a single column, the stacked sheet's chart row floors at 250px (so a drawn 390px page measures 517px for the strip; 340px is the no-script height), legend columns narrow to 2.7rem, the install window drops its platform label, the runlog reveals its `← scrolls sideways` hint, and the docs row drops its arrow.

**The clear field.** The pens never draw under the printed labels. The recorder builds the chart's own box inside the strip (`.strip__field`), draws the traces into it and scales the `viewBox` to *that* box; the box is transparent to layout (`display: contents`) wherever the chips already stay clear — every width from 1280px up, where the strip is the chart it has always been — and where they cannot, the sheet prints in three bands, caption → chart → foot, all static, the chart's row taking the slack (`flex: 1 1 auto`) with a floor of `clamp(250px, 38vw, 300px)` (250px on the 901–1279px panel, where the sheet's height is still set by the legend). Measured clearance from the foot's top edge to the lowest lane baseline: 59–62px across 901 / 1024 (against −16.2 and −18.2 before the fix, when the foot's two-row wrap sat inside the last lane), 62–69px across 1100 / 1200 / 1279, and 29–31px at ≥1280, where nothing moved. At 390px the strip measures 517px, the chart's box 117–367px, and the lane baselines 172/203/234/265/296/327 — the three chips and the box do not intersect, every lane's blocked ink measures 0, and the failed lane's visible alarm pixels went 6 → 969 (its ✗ and `exit 128` used to be covered); the drawn phone page grew 12256 → 12591px. Crossing 1279→1280 is a deliberate step: the trace scale changes once — the field is ≈314px at 1279 and the whole 444.5px panel at 1280 — and each side is correct for its layout.

Two themes, one layout. `:root[data-theme="dark"]` re-resolves the desk tokens, `color-scheme` follows the theme on the root, an inline `<head>` script sets the attribute before the stylesheet so the first paint is already the right paper, and `@media print` forces the sheet back (`--paper #f5f6f4`, `--ink #15161a`, `color-scheme: light`, the body on paper-fibre). The dark desk is `--plate #191a1d` under `graphite-plate.jpg`, its panel steps up to `--plate-2 #22242a` on `graphite-plate-2.jpg`, and the paper objects keep their ramp — the strip stays `#f5f6f4` + chart-paper, the runlog and the official install window become `#fff` + paper-fibre — while at ≤620px the legend goes back to being the surface it stands on, which in dark is the bare desk itself. Geometry is identical in both themes: 388 leaf nodes measure the same, and the dark footer draws its 1px hairline as `inset 0 1px 0` instead of a border-top so the dark page is not a pixel taller (heights 8716 desktop and 12591 on the phone after the sheet fix, the same in both themes); the shipped header went from 75.98 to exactly 76px (132 at 390). Overflow is 0 across 1440 / 1280 / 1100 / 900 / 620 / 390 × five pages × both themes, and the light theme's pixels are otherwise untouched — a reduced-motion pair diff shows nothing moved but the new header control and the three strip chips (1440 mean |Δ| 0.033, 0.052% of pixels above 8 LSB; 390 mean 0.150, 0.264%). The head band carries the wordmark, the four nav links, the 34px theme keycap and the language group; it measures exactly 76px on one row at 1440 / 1000 / 620 and folds to two rows (132px) at 390px, with no horizontal overflow — 561px up keeps one row on the tighter 16px column gap, and at ≤560px the second row is always the paired theme switch + language group, never an orphan at any width 380–620.
...
**The Clear-Field Rule.** Labels and pens never share a surface. The strip's 12–13px chips print in bands above and below the chart's own box — the `.strip__field` the recorder measures and scales the pens to — and where the sheet cannot keep them clear (stacked under 901px, and on the panel's narrow 901–1279px column where the foot wraps to two rows) it gives the chart a row of its own rather than letting a chip sit on a lane. The measure is clearance, not luck: 59–69px at ≤1279px, 0 blocked ink in every lane at 390px, and ≥1280 untouched at 29–31px.
...
Character: the recorder's pen carriage crossing the paper. A 1px Graphite head at 40% opacity with a 14×8 Graphite block travels left to right over 2400ms on a requestAnimationFrame cubic ease-out, while each trace draws itself with `stroke-dashoffset` 1 → 0 (paths carry `pathLength="1"`), one lane staggered 60ms, easing `cubic-bezier(.22, .61, .24, 1)`. **The pen stamps its own marks**: a flag, a cross or an exit code starts off the paper (inline `opacity: 0`) and lands when the carriage reaches its x — `recorder.js` inverts the sweep's own ease (`at = 1 − (1 − fraction)^(1/3)`, `fraction = (x − left)/span`) and delays the 260ms linear fade by `at × 2400ms × 0.94`, measured 678ms for the hero's ✗ and 698ms for its `128`, within a millisecond of the formula — and the mark then stays on the paper (a mark with no `data-stamp-x` falls back to 82% of the sweep). The carriage parks at the right edge and fades out over 200ms. It fires once when the strip first crosses 35% of the viewport, redraws on resize (settled), and can be replayed by the **replay control**: a punched white chip with a 12px drawn glyph and a label in panel caps, inverting to Graphite/Paper on hover in 140ms. Under `prefers-reduced-motion: reduce` the script settles immediately — traces drawn, marks stamped, carriage parked and hidden — the replay control re-settles instead of animating, and `beforeprint` settles every drawn strip (printing never fires the observer), so the page goes to paper with its marks on it: the hero's animated and reduced-motion screenshots are now bit-identical, and its peak alarm pixels went 3946 → 4136 once the marks stayed on the paper.
...
- **Do** keep the printed labels and the drawing in separate space: the recorder draws into the chart's own box (`.strip__field`) and scales the pens to that box, and the sheet gives the chart a row of its own wherever the caption and foot chips would sit on a lane (≤1279px). The measure is clearance, not luck — 59–69px at ≤1279px, 0 blocked ink per lane at 390px, ≥1280 untouched.
- **Do** let the pen stamp its own marks: a flag, cross or exit code waits off the paper and lands when the carriage reaches its x (the sweep's own ease, inverted), then stays there — and settle them for reduced motion, replay and `beforeprint`, never sweep them back off the sheet.
- **Do** honor reduced motion by settling the strip to its final state — drawn traces, visible marks, parked carriage — rather than suppressing the drawing, and settle before print, where the observer never fires.
